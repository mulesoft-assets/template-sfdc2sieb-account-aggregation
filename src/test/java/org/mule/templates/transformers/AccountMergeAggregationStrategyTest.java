...
public class AccountMergeAggregationStrategyTest extends AbstractTemplateTestCase {
	...
	private static final Logger log = LogManager.getLogger(AccountMergeAggregationStrategyTest.class);
	...
	public void testAggregate() throws Exception {
		...
		MuleEvent testEventSiebel = getTestEvent("");
		
		testEventSFDC.getMessage().setPayload(accountsSFDC.iterator());
		testEventSiebel.getMessage().setPayload(accountsSiebel.iterator());
		
		List<MuleEvent> testEvents = new ArrayList<MuleEvent>();
		testEvents.add(testEventSFDC);
		testEvents.add(testEventSiebel);
		
		AggregationContext aggregationContext = new AggregationContext(testOriginalEvent, testEvents);
		
		AccountMergeAggregationStrategy aggregationStrategy = new AccountMergeAggregationStrategy();
		List<Map<String, String>> mergedList = Lists.newArrayList((Iterator<Map<String, String>>) aggregationStrategy.aggregate(aggregationContext).getMessage().getPayload());
